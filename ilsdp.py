...
class ILSDP():
    def __init__(self, distances, perts=20, vertex=False):
        ...
        self.mask = [1] * int(self.dist_len / 2 * perts / 100)
        self.mask.extend([0] * (int(self.dist_len / 2) - int(self.dist_len / 2 * perts / 100)))
...
def main():
    ...
    for instance in [KROA, KROB]:
        ...
        label = f'ILSDP | {instance} | distance: {ls.best_cost}'
        save_name = f'ilsdp_{instance}'
